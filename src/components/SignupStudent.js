...
function SignupStudent({ onSignup }) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 350, margin: '40px auto', background: '#fff', padding: 32, borderRadius: 12, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
      <h2 style={{ color: 'var(--color-primary)' }}>Student Sign Up</h2>
      <input name="roll" type="text" placeholder="Roll Number *" value={form.roll} onChange={handleChange} required style={{ padding: 10, borderRadius: 6, border: '1px solid #ddd' }} />
      {errors.roll && <span style={{ color: 'red', fontSize: '0.95em' }}>{errors.roll}</span>}
      ...
      {Array.isArray(errors.password) && errors.password.length > 0 && (
        <ul style={{ color: 'red', fontSize: '0.95em', margin: '8px 0 0 0', paddingLeft: 18 }}>
          ...
          {errors.password.map((rule, idx) => <li key={idx}>{rule}</li>)}
