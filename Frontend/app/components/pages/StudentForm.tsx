...
export default function StudentForm() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(120deg, #f3f4f6 60%, #e3e8ff 100%)', paddingTop: 32, paddingBottom: 32 }}>
      ...
      <form onSubmit={handleSectionSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 24, width: '80%', margin: '32px auto 0 auto', background: '#fff', padding: 40, borderRadius: 18, boxShadow: '0 4px 24px rgba(0,0,0,0.10)' }}>
        ...
        <button type="submit" style={{ marginTop: 18, fontWeight: 600, fontSize: 16, letterSpacing: 0.5, padding: '12px 24px', background: 'var(--color-primary)', color: '#fff', border: 'none', borderRadius: 6, cursor: 'pointer' }}>{sectionIdx < sections.length - 1 ? 'Save & Next' : 'Submit'}</button>
      </form>
    </div>
  );
}
