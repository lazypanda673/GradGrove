...
interface StudentFormInlineProps {
  onSubmit: (data: FormData) => void;
  onCancel: () => void;
}

// Inline version of StudentForm with cancel and submit callback
function StudentFormInline({ onSubmit, onCancel }: StudentFormInlineProps) {
  ...
  if (completed) {
    return (
      <div style={{ maxWidth: 600, margin: '0 auto', background: '#fff', padding: 32, borderRadius: 12, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', textAlign: 'center' }}>
        ...
        <button style={{ marginTop: 18, background: 'var(--color-primary)', color: '#fff', borderRadius: 6, fontWeight: 600, padding: '10px 24px', border: 'none', cursor: 'pointer' }} onClick={onCancel}>Close</button>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: '1px', background: 'linear-gradient(120deg, #f3f4f6 60%, #e3e8ff 100%)', paddingTop: 8, paddingBottom: 8 }}>
      ...
      <form onSubmit={handleSectionSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 24, width: '80%', margin: '24px auto 0 auto', background: '#fff', padding: 32, borderRadius: 18, boxShadow: '0 4px 24px rgba(0,0,0,0.10)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
          ...
          <button type="submit" style={{ background: 'var(--color-primary)', color: '#fff', borderRadius: 6, fontWeight: 600, fontSize: 16, letterSpacing: 0.5, padding: '12px 24px', border: 'none', cursor: 'pointer' }}>{sectionIdx < sections.length - 1 ? 'Save & Next' : 'Submit'}</button>
        </div>
      </form>
    </div>
  );
              ...
              <div style={{ marginTop: 16 }}>
                {!showForm ? (
                  <AnimatedElement animation="slideInUp" delay={400}>
                    <button className="hover-scale transition-all" style={{ background: 'var(--color-primary)', color: '#fff', padding: '12px 32px', borderRadius: 8, fontWeight: 600, fontSize: 17, boxShadow: '0 1px 4px rgba(0,0,0,0.10)', border: 'none', cursor: 'pointer' }} onClick={() => setShowForm(true)}>
                      Fill Student Form
                    </button>
                  </AnimatedElement>
                ) : null}
            ...
            <>
              ...
              <AnimatedElement animation="slideInUp" delay={400}>
                <div style={{ marginTop: 16, color: '#555' }}>
                  ...
                  <button disabled className="transition-all" style={{ background: '#ddd', color: '#888', cursor: 'not-allowed', marginTop: 12, borderRadius: 6, padding: '8px 16px', border: 'none' }}>Already Filled</button>
                </div>
              </AnimatedElement>
            </>
          )}
